...
CREATE INDEX IF NOT EXISTS idx_portfolio_user_id
  ON portfolio(user_id) WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_portfolio_status
  ON portfolio(status) WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_portfolio_created_at
  ON portfolio(created_at) WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_portfolio_updated_at
  ON portfolio(updated_at) WHERE deleted_at IS NULL;

-- Index for JSONB data searches
CREATE INDEX IF NOT EXISTS idx_portfolio_data_gin
  ON portfolio USING gin(data) WHERE deleted_at IS NULL;
...
CREATE TRIGGER portfolio_timestamp_trigger
