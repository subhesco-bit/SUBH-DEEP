...
CREATE INDEX IF NOT EXISTS idx_secrets_user_id
  ON secrets(user_id) WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_secrets_status
  ON secrets(status) WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_secrets_created_at
  ON secrets(created_at) WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_secrets_updated_at
  ON secrets(updated_at) WHERE deleted_at IS NULL;

-- Index for JSONB data searches
CREATE INDEX IF NOT EXISTS idx_secrets_data_gin
  ON secrets USING gin(data) WHERE deleted_at IS NULL;
