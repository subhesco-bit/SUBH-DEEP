...
async function sendSMSOTP(userId, phoneNumber) {
  ...
  logger.info('SMS OTP generated', { userId, phoneNumber });
...
async function verifySMSOTP(userId, otp) {
  ...
  const res = await pg.query(
    `SELECT * FROM mfa_otp 
    WHERE user_id = $1 AND method = 'sms' AND otp_hash = $2 AND expires_at > NOW() AND used = false
...
async function sendEmailOTP(userId, email) {
  ...
  logger.info('Email OTP generated', { userId, email });
...
async function verifyEmailOTP(userId, otp) {
  ...
  const res = await pg.query(
    `SELECT * FROM mfa_otp 
    WHERE user_id = $1 AND method = 'email' AND otp_hash = $2 AND expires_at > NOW() AND used = false
     ORDER BY created_at DESC LIMIT 1`,
    [userId, hashedOTP],
  );
