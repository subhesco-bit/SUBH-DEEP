...
router.use((req, res, next) => {
  if (!M041Service) {
    try {
      ...
      console.warn('M041Service not available:', e.message);
      return res.status(503).json({ error: 'M041 service not available' });
    }
  }
  next();
});

// Create service instance with dependencies from request context
const getService = (req) => {
  if (!req.app.locals.m041Service) {
    const pool = req.app.locals.db || global.db;
    const logger = req.app.locals.logger || console;
    req.app.locals.m041Service = new M041Service(pool, logger);
  }
  return req.app.locals.m041Service;
};
