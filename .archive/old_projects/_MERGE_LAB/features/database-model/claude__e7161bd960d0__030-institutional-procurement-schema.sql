...
CREATE INDEX IF NOT EXISTS idx_procurement_tenders_number ON procurement_tenders(tender_number);
CREATE INDEX IF NOT EXISTS idx_procurement_tenders_institution ON procurement_tenders(institution_id);
CREATE INDEX IF NOT EXISTS idx_procurement_tenders_type ON procurement_tenders(tender_type);
CREATE INDEX IF NOT EXISTS idx_procurement_tenders_status ON procurement_tenders(status);
...
-- Reconciliation 2026-08-04: tender_bids is also defined in an earlier migration,
...
ALTER TABLE tender_bids ADD COLUMN IF NOT EXISTS submission_date TIMESTAMP;
...
-- Reconciliation 2026-08-04: demand_forecasts is also defined in an earlier migration,
-- so the CREATE TABLE above is a no-op and this file's extra columns were
-- silently lost — surfacing later as "column ... does not exist" on its
-- indexes. These ALTERs make this file's expected shape real either way.
ALTER TABLE demand_forecasts ADD COLUMN IF NOT EXISTS budget_constraints JSONB;
ALTER TABLE demand_forecasts ADD COLUMN IF NOT EXISTS confidence_level DECIMAL(5,2);
ALTER TABLE demand_forecasts ADD COLUMN IF NOT EXISTS enrollment_data JSONB;
ALTER TABLE demand_forecasts ADD COLUMN IF NOT EXISTS forecast_method VARCHAR(100);
ALTER TABLE demand_forecasts ADD COLUMN IF NOT EXISTS forecast_period VARCHAR(50);
ALTER TABLE demand_forecasts ADD COLUMN IF NOT EXISTS forecast_type VARCHAR(50);
...
ALTER TABLE demand_forecasts ADD COLUMN IF NOT EXISTS product_category VARCHAR(100);
...
ALTER TABLE demand_forecasts ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP;

CREATE INDEX IF NOT EXISTS idx_demand_forecasts_institution ON demand_forecasts(institution_id);
CREATE INDEX IF NOT EXISTS idx_demand_forecasts_type ON demand_forecasts(forecast_type);
CREATE INDEX IF NOT EXISTS idx_demand_forecasts_category ON demand_forecasts(product_category);
...
CREATE INDEX IF NOT EXISTS idx_menu_plans_institution ON menu_plans(institution_id);
CREATE INDEX IF NOT EXISTS idx_menu_plans_type ON menu_plans(plan_type);
CREATE INDEX IF NOT EXISTS idx_menu_plans_dates ON menu_plans(start_date, end_date);
CREATE INDEX IF NOT EXISTS idx_menu_plans_status ON menu_plans(status);
...
CREATE INDEX IF NOT EXISTS idx_nutrition_compliance_institution ON nutrition_compliance(institution_id);
CREATE INDEX IF NOT EXISTS idx_nutrition_compliance_type ON nutrition_compliance(compliance_type);
CREATE INDEX IF NOT EXISTS idx_nutrition_compliance_date ON nutrition_compliance(assessment_date);
...
CREATE INDEX IF NOT EXISTS idx_supply_contracts_number ON supply_contracts(contract_number);
CREATE INDEX IF NOT EXISTS idx_supply_contracts_institution ON supply_contracts(institution_id);
CREATE INDEX IF NOT EXISTS idx_supply_contracts_supplier ON supply_contracts(supplier_id);
...
CREATE INDEX IF NOT EXISTS idx_quality_inspections_contract ON quality_inspections(contract_id);
CREATE INDEX IF NOT EXISTS idx_quality_inspections_shipment ON quality_inspections(shipment_id);
CREATE INDEX IF NOT EXISTS idx_quality_inspections_status ON quality_inspections(approval_status);
...
CREATE INDEX IF NOT EXISTS idx_settlement_records_number ON settlement_records(settlement_number);
CREATE INDEX IF NOT EXISTS idx_settlement_records_contract ON settlement_records(contract_id);
CREATE INDEX IF NOT EXISTS idx_settlement_records_invoice ON settlement_records(invoice_id);
CREATE INDEX IF NOT EXISTS idx_settlement_records_date ON settlement_records(settlement_date);
...
DROP TRIGGER IF EXISTS update_procurement_tenders_updated_at ON procurement_tenders;
...
DROP TRIGGER IF EXISTS update_menu_plans_updated_at ON menu_plans;
CREATE TRIGGER update_menu_plans_updated_at BEFORE UPDATE ON menu_plans
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_nutrition_compliance_updated_at ON nutrition_compliance;
CREATE TRIGGER update_nutrition_compliance_updated_at BEFORE UPDATE ON nutrition_compliance
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_supply_contracts_updated_at ON supply_contracts;
