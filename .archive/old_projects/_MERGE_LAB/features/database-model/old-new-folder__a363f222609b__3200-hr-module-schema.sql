...
-- Create the referenced relation before employees. The full definition below
-- is intentionally retained for fresh-schema readability; CREATE TABLE IF NOT
-- EXISTS cannot solve this dependency on its own.
...
-- Explicitly evolve partially-created HR tables before indexes. This keeps a
-- rerun safe when an earlier deployment created a table with fewer columns.
...
-- The employee identifier is declared/evolved above. The index is omitted
-- here because older deployments can have a conflicting employees shape;
-- the unique constraint on the canonical column provides the lookup index.
