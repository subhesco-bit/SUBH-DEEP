...
export default function M056Page() {
  ...
  const fetchData = useCallback(async (page = 1) => {
    ...
    try {
      ...
      const response = await fetch(`/api/m056?${params}`);
  ...
  const handleSearch = useCallback(async (e) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`/api/m056/search?q=${encodeURIComponent(searchQuery)}`);
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    try {
      ...
      const url = editingId ? `/api/m056/${editingId}` : `/api/m056`;
  ...
  const handleDelete = useCallback(async (id) => {
    ...
    try {
      const response = await fetch(`/api/m056/${id}`, { method: 'DELETE' });
