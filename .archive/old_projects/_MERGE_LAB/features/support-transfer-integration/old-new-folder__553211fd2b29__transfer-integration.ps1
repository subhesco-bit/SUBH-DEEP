...
$BackendServices = (Get-ChildItem -Path "backend\src\services" -Filter "*.js" -Recurse | Where-Object { $_.Name -ne "index.js" }).Count
...
"@
try {
    $TransferReport | Out-File (Join-Path $AiDirectory "TRANSFER_READINESS_REPORT.txt") -Encoding UTF8
} catch {
    Write-Warning "Report persistence unavailable: $($_.Exception.Message)"
}
...
try {
    ...
    Write-Warning "Backup directory persistence unavailable: $($_.Exception.Message)"
}
...
try {
    ...
    Write-Warning "Deletion manifest persistence unavailable: $($_.Exception.Message)"
}
