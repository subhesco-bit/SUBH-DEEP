...
function isPlaceholderSecret(value) {
  ...
  return /^(change_|your_|replace_|example|password$|postgres$)/i.test(value.trim());
}
...
class UnifiedConfigService {
  ...
  getDefaultConfig() {
    return {
      ...
      claudeAI: {
        provider: process.env.AI_DEFAULT_PROVIDER || 'claude',
  ...
  validateConfig() {
    ...
    if (this.config.project.environment === 'production' && isPlaceholderSecret(this.config.database.password)) {
      errors.push('A non-placeholder DB_PASSWORD is required in production');
    }
    
    if (this.config.project.environment === 'production' && isPlaceholderSecret(this.config.security.session.secret)) {
      errors.push('A non-placeholder SESSION_SECRET is required in production');
    }
