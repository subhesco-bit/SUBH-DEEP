...
// villageProfileService — minimal in-memory scaffold.
let _items = [];
let _nextId = 1;
...
router.get('/', async (req, res) => {
  try {
    ...
    logger.warn('villageProfileService list query failed, falling back to in-memory store', { error: error.message });
    res.json({ success: true, data: _items });
  }
});
...
function setupRoutes(app) {
  app.use('/api/v1/village-profile', router);
}
