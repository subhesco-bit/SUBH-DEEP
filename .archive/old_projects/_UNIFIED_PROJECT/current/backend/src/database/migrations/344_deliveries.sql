...
CREATE TABLE IF NOT EXISTS deliveries (
  ...
  -- Constraints
  CONSTRAINT deliveries_user_fk FOREIGN KEY (user_id)
    REFERENCES users(id) ON DELETE CASCADE
);

-- Create indexes for common queries
CREATE INDEX IF NOT EXISTS idx_deliveries_user_id
  ON deliveries(user_id) WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_deliveries_status
  ON deliveries(status) WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_deliveries_created_at
  ON deliveries(created_at) WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_deliveries_updated_at
  ON deliveries(updated_at) WHERE deleted_at IS NULL;

-- Index for JSONB data searches
CREATE INDEX IF NOT EXISTS idx_deliveries_data_gin
  ON deliveries USING gin(data) WHERE deleted_at IS NULL;
...
CREATE TRIGGER deliveries_timestamp_trigger
