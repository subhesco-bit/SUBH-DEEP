...
CREATE TABLE IF NOT EXISTS recommendations (
  ...
  -- Constraints
  CONSTRAINT recommendations_user_fk FOREIGN KEY (user_id)
...
CREATE INDEX IF NOT EXISTS idx_recommendations_status
  ON recommendations(status) WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_recommendations_created_at
  ON recommendations(created_at) WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_recommendations_updated_at
  ON recommendations(updated_at) WHERE deleted_at IS NULL;

-- Index for JSONB data searches
CREATE INDEX IF NOT EXISTS idx_recommendations_data_gin
  ON recommendations USING gin(data) WHERE deleted_at IS NULL;
