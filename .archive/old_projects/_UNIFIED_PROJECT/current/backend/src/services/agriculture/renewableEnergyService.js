...
// renewableEnergyService — minimal in-memory scaffold.
let _items = [];
let _nextId = 1;
...
router.get('/', async (req, res) => {
  try {
    ...
    logger.warn('renewableEnergyService list query failed, falling back to in-memory store', { error: error.message });
    res.json({ success: true, data: _items });
  }
});
