...
export default function M120Page() {
  ...
  const fetchData = useCallback(async (page = 1) => {
    ...
    try {
      ...
      const response = await fetch(`/api/m120?${params}`);
  ...
  const handleSearch = useCallback(async (e) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`/api/m120/search?q=${encodeURIComponent(searchQuery)}`);
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    try {
      ...
      const url = editingId ? `/api/m120/${editingId}` : `/api/m120`;
  ...
  const handleDelete = useCallback(async (id) => {
    ...
    try {
      const response = await fetch(`/api/m120/${id}`, { method: 'DELETE' });
