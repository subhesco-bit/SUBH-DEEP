...
//#region node_modules/.nitro/vite/services/ssr/assets/match-O6S8HVhu.js
/**
...
function contractCards() {
	return BRIDGES.filter((b) => b.status !== "living" && (b.signal.includes("harvest.completed") || b.signal.includes("lot.mint") || b.signal.includes("order.settled") || b.signal.includes("fdi.") || b.signal.includes("weather") || b.signal.includes("scheme.") || b.signal.includes("energy.cloud") || b.signal.includes("graph.") || b.signal.includes("module.") || b.signal.includes("agentic."))).slice(0, 16).map((b) => ({
...
function queryLibraryKnowledge(query, opts = {}, cards = libraryCatalog()) {
	...
	for (const card of cards) {
		...
		let score = 0;
		if (terms.length === 0) score = opts.organId && card.organs.includes(opts.organId) ? 1 : .4;
		else {
			for (const term of terms) if (card.id.toLowerCase() === term) score += 3;
			else if (title.includes(term)) score += 2;
			else if (signal.includes(term)) score += 2.5;
			else if (hay.includes(term)) score += 1;
			if (opts.organId && card.organs.includes(opts.organId)) score += .5;
		}
		if (terms.length === 0 || score > 0) hits.push({
			...card,
			relevance: terms.length ? score / (terms.length * 2 + .5) : score
		});
	}
...
//#endregion
export { queryLibraryKnowledge as i, cardsForOrgan as n, libraryCatalog as r, bindingCount as t };
