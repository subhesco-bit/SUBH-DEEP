...
CREATE INDEX IF NOT EXISTS idx_events_user_id
  ON events(user_id) WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_events_status
  ON events(status) WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_events_created_at
  ON events(created_at) WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_events_updated_at
  ON events(updated_at) WHERE deleted_at IS NULL;

-- Index for JSONB data searches
CREATE INDEX IF NOT EXISTS idx_events_data_gin
  ON events USING gin(data) WHERE deleted_at IS NULL;
