-- Folded from backend/src/modules/M024/model.sql (AUDIT_DB.md Finding 11/13 remediation)
-- Farmer Groups Schema (M024) / -- Farmer producer groups and cooperatives management
...
CREATE INDEX IF NOT EXISTS idx_farmer_groups_state ON farmer_groups(state);

CREATE INDEX IF NOT EXISTS idx_farmer_groups_status ON farmer_groups(status);

CREATE INDEX IF NOT EXISTS idx_group_memberships_group ON group_memberships(group_id);

CREATE INDEX IF NOT EXISTS idx_group_memberships_farmer ON group_memberships(farmer_id);
...
CREATE INDEX IF NOT EXISTS idx_group_finances_group ON group_finances(group_id);

CREATE INDEX IF NOT EXISTS idx_group_finances_date ON group_finances(transaction_date);
