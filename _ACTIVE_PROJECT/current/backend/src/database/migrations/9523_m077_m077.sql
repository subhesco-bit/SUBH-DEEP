-- Folded from backend/src/modules/M077/model.sql (AUDIT_DB.md Finding 11/13 remediation)
-- ﻿-- Water Quality Monitoring Schema (M077) / -- Real-time water quality tracking and compliance monitoring
...
CREATE INDEX IF NOT EXISTS idx_water_quality_compliance ON water_quality_measurements(compliance_status);

CREATE INDEX IF NOT EXISTS idx_water_sources_location ON water_sources(location_id);

CREATE INDEX IF NOT EXISTS idx_quality_alerts_location ON quality_alerts(location_id);

CREATE INDEX IF NOT EXISTS idx_quality_alerts_status ON quality_alerts(status);

CREATE INDEX IF NOT EXISTS idx_treatment_recommendations_location ON treatment_recommendations(location_id);
