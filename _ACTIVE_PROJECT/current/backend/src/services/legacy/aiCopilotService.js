...
/**
 * List the current user's copilot sessions, optionally filtered by type.
 */
router.get('/sessions', authMiddleware, async (req, res) => {
  try {
    const { copilot_type: copilotType, limit = 25 } = req.query;

    const result = await pool.query(
      `SELECT id, copilot_type, context, session_metadata, status,
              message_count, created_at, updated_at, last_activity, ended_at
         FROM copilot_sessions
        WHERE user_id = $1
          AND ($2::text IS NULL OR copilot_type = $2)
        ORDER BY last_activity DESC, updated_at DESC
        LIMIT $3`,
      [req.user.id, copilotType || null, Math.min(Number(limit) || 25, 100)],
    );

    res.json({ sessions: result.rows });
  } catch (error) {
    logger.error('List copilot sessions error', { error: error.message, stack: error.stack });
    res.status(500).json({ error: 'Failed to list copilot sessions' });
  }
});
