...
async function exchangeCodeForTokens(providerConfig, code, redirectUri) {
  if (!providerConfig.token_url || !providerConfig.client_id || !providerConfig.client_secret) {
    logger.warn('SSO token exchange skipped because provider configuration is incomplete', {
  ...
  const response = await axios.post(providerConfig.token_url, payload.toString(), {
...
async function getUserInfoFromProvider(providerConfig, accessToken) {
  if (!providerConfig.user_info_url || !accessToken) {
    logger.warn('SSO user-info lookup skipped because provider configuration or token is missing', {
      provider: providerConfig.provider_name,
    });
    return null;
  }

  const response = await axios.get(providerConfig.user_info_url, {
...
function generateSAMLRequest(providerConfig, redirectUri) {
  throw new Error('SAML request generation requires a configured SAML implementation before production use');
}

async function validateSAMLResponse(providerConfig, samlResponse) {
  logger.warn('SAML response validation is not enabled for this provider', {
    provider: providerConfig.provider_name,
  });
  return null;
}
