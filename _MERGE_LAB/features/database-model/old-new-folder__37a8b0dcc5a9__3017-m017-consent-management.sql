...
-- Migration 014 already creates consents with a different column set. Add
-- consent-management fields explicitly so the later migration is idempotent.
ALTER TABLE consents ADD COLUMN IF NOT EXISTS status VARCHAR(20) DEFAULT 'active';
ALTER TABLE consents ADD COLUMN IF NOT EXISTS consent_data JSONB DEFAULT '{}';
ALTER TABLE consents ADD COLUMN IF NOT EXISTS expires_at TIMESTAMP;
ALTER TABLE consents ADD COLUMN IF NOT EXISTS valid_from TIMESTAMP DEFAULT CURRENT_TIMESTAMP;
ALTER TABLE consents ADD COLUMN IF NOT EXISTS valid_until TIMESTAMP DEFAULT CURRENT_TIMESTAMP + INTERVAL '365 days';
