-- FK TYPE FIX 2026-08-04: 1 column(s) in this file declared INTEGER while
...
CREATE INDEX IF NOT EXISTS idx_food_nutrition_name ON food_nutrition_profiles USING gin(to_tsvector('english', food_name));
CREATE INDEX IF NOT EXISTS idx_food_nutrition_group ON food_nutrition_profiles(food_group);
...
CREATE INDEX IF NOT EXISTS idx_product_nutrition_scores_product ON product_nutrition_scores(product_id);
...
-- Reconciliation 2026-08-04: dietary_profiles is also defined in an earlier migration,
...
ALTER TABLE dietary_profiles ADD COLUMN IF NOT EXISTS name VARCHAR(100);
...
ALTER TABLE dietary_profiles ADD COLUMN IF NOT EXISTS target_audience VARCHAR(100);

CREATE TABLE IF NOT EXISTS nutrition_recommendations (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID REFERENCES users(id),
    dietary_profile_id UUID REFERENCES dietary_profiles(id),
...
DROP TRIGGER IF EXISTS update_food_nutrition_profiles_updated_at ON food_nutrition_profiles;
