...
function WbsNode({ node, depth = 0 }) {
  return (
    <>
      <tr>
        <td style={{ paddingLeft: 8 + depth * 20, borderBottom: '1px solid #eaeef2', padding: '6px 8px' }}>
          {node.wbs_code} — {node.wbs_name}
        </td>
        <td style={{ textAlign: 'left', borderBottom: '1px solid #eaeef2', padding: '6px 8px' }}>{node.status}</td>
        <td style={{ textAlign: 'right', borderBottom: '1px solid #eaeef2', padding: '6px 8px' }}><Rupees value={node.rollupPlanned} /></td>
        <td style={{ textAlign: 'right', borderBottom: '1px solid #eaeef2', padding: '6px 8px' }}><Rupees value={node.rollupActual} /></td>
        <td style={{ textAlign: 'right', borderBottom: '1px solid #eaeef2', padding: '6px 8px', color: node.variance > 0 ? '#cf222e' : '#1a7f37' }}>
          ...
          {selectedProjectId && (
            <>
              <Section title="Work breakdown structure"
                ...
                <form onSubmit={submitWbs} style={{ display: 'flex', gap: 10, alignItems: 'flex-end', flexWrap: 'wrap', padding: 12, background: '#f6f8fa', border: '1px solid #d0d7de', borderRadius: 6 }}>
                ...
                {wbsMsg && <p role="alert" style={{ color: '#cf222e', fontSize: 13, marginTop: 8 }}>{wbsMsg}</p>}

                {rollupError ? (
                  <p role="alert" style={{ color: '#cf222e', marginTop: 12 }}>{rollupError}</p>
                ) : (
                  <div className="overflow-x-auto" style={{ marginTop: 12 }}>
                    {Array.isArray(rollup) && rollup.length > 0 ? (
                      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
                        ...
                        <thead>
                          <tr>
                            <th scope="col" style={{ textAlign: 'left', borderBottom: '2px solid #d0d7de', padding: '6px 8px' }}>Element</th>
                            <th scope="col" style={{ textAlign: 'left', borderBottom: '2px solid #d0d7de', padding: '6px 8px' }}>Status</th>
                            <th scope="col" style={{ textAlign: 'right', borderBottom: '2px solid #d0d7de', padding: '6px 8px' }}>Planned (rollup)</th>
                            <th scope="col" style={{ textAlign: 'right', borderBottom: '2px solid #d0d7de', padding: '6px 8px' }}>Actual (rollup)</th>
                            <th scope="col" style={{ textAlign: 'right', borderBottom: '2px solid #d0d7de', padding: '6px 8px' }}>Variance</th>
                          </tr>
                        </thead>
                        <tbody>
                ...
                {wbsList.length > 0 && (
                  <div style={{ marginTop: 12, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    {wbsList.map((w) => (
                      <span key={w.id} style={{ fontSize: 12, border: '1px solid #d0d7de', borderRadius: 4, padding: '2px 6px' }}>
                        {w.wbs_code}:
              ...
              <Section title="Milestones">
                {milestoneSummary && (
                  <p style={{ fontSize: 14 }}>
                    ...
                    {' · '}Overdue <strong style={{ color: milestoneSummary.counts.overdue > 0 ? '#cf222e' : undefined }}>
                ...
                <form onSubmit={submitMilestone} style={{ display: 'flex', gap: 10, alignItems: 'flex-end', flexWrap: 'wrap', padding: 12, background: '#f6f8fa', border: '1px solid #d0d7de', borderRadius: 6 }}>
                ...
                {milestoneMsg && <p role="alert" style={{ color: '#cf222e', fontSize: 13, marginTop: 8 }}>{milestoneMsg}</p>}

                {milestonesError ? (
                  <p role="alert" style={{ color: '#cf222e', marginTop: 12 }}>{milestonesError}</p>
                ) : (
                  <DataTable
                    caption="Milestones"
              ...
              <Section title="Project budget vs. actual"
                ...
                {budgetVsActual?.error ? (
                  <p role="alert" style={{ color: '#cf222e' }}>{budgetVsActual.error}</p>
                ) : budgetVsActual ? (
                  <p style={{ fontSize: 15 }}>
                    ...
                    {' · '}
                    <strong style={{ color: budgetVsActual.variance > 0 ? '#cf222e' : '#1a7f37' }}>
