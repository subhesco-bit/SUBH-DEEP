...
const defaultProvider = process.env.AI_DEFAULT_PROVIDER || 'claude';
  ...
  async executeAgentTask(agentName, task, context = {}) {
    ...
    try {
      ...
      if (agent.model === 'gpt-4') {
        ...
        const coordinated = await claudeAICoordinator.coordinateAIRequest({
          requestType: 'general',
          query: JSON.stringify({
            systemPrompt: agent.system_prompt,
            task,
            context,
            memory: messages.slice(1)
          })
        });
        response = { choices: [{ message: { content: coordinated.response, tool_calls: [] } }] };
      } else {
