...
class ClaudeAICoordinator {
  constructor() {
    ...
    const apiKey = claudeConfig.apiKey || process.env.ANTHROPIC_API_KEY || process.env.CLAUDE_API_KEY;

    this.anthropic = apiKey ? new Anthropic({ apiKey }) : null;
    this.provider = claudeConfig.provider || 'claude';
  ...
  async processAgentRequest(agent, query, context, sessionContext) {
    if (!this.anthropic) {
      throw new Error('Claude AI is not configured. Set ANTHROPIC_API_KEY or CLAUDE_API_KEY before making AI requests.');
    }
