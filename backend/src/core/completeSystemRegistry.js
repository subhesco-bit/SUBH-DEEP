...
/** Canonical evidence registry for Platform, Domain and Enterprise systems. */
const SYSTEMS = Object.freeze([
  ...
  ['S07', 'Rural Finance, Government Schemes & Cooperative', 'Domain', ['ruralfinance','scheme','subsidy','cooperative','grant','benefit']],
...
function collectEvidence(projectRoot) {
  const path = require('path'); const evidence = [];
  const sources = [
  ...
  for (const [kind, root, predicate] of sources) for (const file of walk(root, predicate)) evidence.push({ kind, path: path.relative(projectRoot, file).replace(/\\/g, '/') });
  return evidence;
}
function classifyEvidence(item) {
  const haystack = normalise(item.path); let best = null; let bestScore = 0;
  for (const [id, name, layer, keywords] of SYSTEMS) {
    ...
    if (score > bestScore) { best = { id, name, layer }; bestScore = score; }
  }
  return best ? { ...best, confidence: Math.min(1, bestScore / 3) } : null;
}
function buildCompleteSystemRegistry(projectRoot = require('path').resolve(__dirname, '../../..')) {
  const evidence = collectEvidence(projectRoot);
  const systems = SYSTEMS.map(([id, name, layer]) => ({ id, name, layer, evidence: evidence.filter(item => classifyEvidence(item)?.id === id), counts: {} }));
  for (const system of systems) for (const item of system.evidence) system.counts[item.kind] = (system.counts[item.kind] || 0) + 1;
  const unclassified = evidence.filter(item => !classifyEvidence(item));
  return { generatedAt: new Date().toISOString(), architecture: ['Platform','Domain','Enterprise'], systems, evidenceCount: evidence.length, unclassifiedCount: unclassified.length, unclassified, sourceOfTruth: 'Existing repository files; no replacement module architecture is created.' };
}
module.exports = { SYSTEMS, collectEvidence, classifyEvidence, buildCompleteSystemRegistry };
