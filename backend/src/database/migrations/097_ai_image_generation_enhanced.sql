-- Enhanced AI Image Generation Schema
-- Supports international-grade image generation with quality tracking
...
CREATE TABLE IF NOT EXISTS ai_generated_images (
  ...
  region VARCHAR(50), -- northeast-india, south-india, global-export
  language VARCHAR(10), -- en, hi, bn, ta, te, es, fr, pt, zh, ja
...
CREATE INDEX idx_images_product_id ON ai_generated_images(product_id);
CREATE INDEX idx_images_farmer_id ON ai_generated_images(farmer_id);
CREATE INDEX idx_images_language ON ai_generated_images(language);
CREATE INDEX idx_images_region ON ai_generated_images(region);
CREATE INDEX idx_images_quality_score ON ai_generated_images(quality_score);
...
CREATE TABLE IF NOT EXISTS ai_image_metadata (
  ...
  prompt_quality_score DECIMAL(5, 2),
  product_accuracy_score DECIMAL(5, 2),
  image_clarity_score DECIMAL(5, 2),
  marketplace_readiness_score DECIMAL(5, 2),
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

CREATE INDEX idx_metadata_image_id ON ai_image_metadata(image_id);
...
CREATE TABLE IF NOT EXISTS ai_image_product_listings (
  ...
  region VARCHAR(50),
  language VARCHAR(10),
  visibility_marketplace BOOLEAN DEFAULT TRUE,
  visibility_website BOOLEAN DEFAULT TRUE,
  visibility_mobile BOOLEAN DEFAULT TRUE,
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
CREATE INDEX idx_optimization_listing ON listing_marketplace_optimization(listing_id);
CREATE INDEX idx_optimization_marketplace ON listing_marketplace_optimization(marketplace_name);

-- Farmer Portfolios Table
...
CREATE INDEX idx_portfolio_farmer_id ON farmer_image_portfolios(farmer_id);
CREATE INDEX idx_portfolio_region ON farmer_image_portfolios(region);
...
CREATE TABLE IF NOT EXISTS farmer_products (
  ...
  status VARCHAR(50), -- pending-images, pending-review, published
...
CREATE INDEX idx_farmer_products_farmer_id ON farmer_products(farmer_id);
CREATE INDEX idx_farmer_products_status ON farmer_products(status);
CREATE INDEX idx_farmer_products_listing ON farmer_products(marketplace_listing_id);

-- Farmer Product Images Table
...
CREATE INDEX idx_farmer_images_product ON farmer_product_images(farmer_product_id);
CREATE INDEX idx_farmer_images_status ON farmer_product_images(status);
CREATE INDEX idx_farmer_images_language ON farmer_product_images(language);
...
CREATE TABLE IF NOT EXISTS listing_performance_metrics (
  ...
  avg_time_on_listing INTEGER DEFAULT 0,
...
CREATE INDEX idx_metrics_listing ON listing_performance_metrics(listing_id);

-- Image Quality Feedback Table
...
CREATE INDEX idx_feedback_image ON image_quality_feedback(image_id);

-- SKU Images Table (for variants)
...
CREATE INDEX idx_sku_images_product ON sku_images(product_id);
CREATE INDEX idx_sku_images_sku ON sku_images(sku);

-- Image Generation Batch Logs
...
CREATE INDEX idx_batch_logs_created ON image_generation_batch_logs(created_at DESC);
...
-- Grant permissions
GRANT SELECT, INSERT, UPDATE ON ALL TABLES IN SCHEMA public TO ebdesign_user;
