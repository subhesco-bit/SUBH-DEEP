...
CREATE TABLE IF NOT EXISTS assets (
  ...
  -- Constraints
  CONSTRAINT assets_user_fk FOREIGN KEY (user_id)
...
-- assets: winner is 000_base_schema.sql
ALTER TABLE assets ADD COLUMN IF NOT EXISTS user_id UUID;
ALTER TABLE assets ADD COLUMN IF NOT EXISTS data JSONB;
...
CREATE INDEX IF NOT EXISTS idx_assets_created_at
  ON assets(created_at) WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_assets_updated_at
  ON assets(updated_at) WHERE deleted_at IS NULL;

-- Index for JSONB data searches
CREATE INDEX IF NOT EXISTS idx_assets_data_gin
  ON assets USING gin(data) WHERE deleted_at IS NULL;
...
CREATE TRIGGER assets_timestamp_trigger
