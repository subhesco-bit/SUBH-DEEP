...
CREATE INDEX IF NOT EXISTS idx_wallets_status
  ON wallets(status) WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_wallets_created_at
  ON wallets(created_at) WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_wallets_updated_at
  ON wallets(updated_at) WHERE deleted_at IS NULL;

-- Index for JSONB data searches
CREATE INDEX IF NOT EXISTS idx_wallets_data_gin
  ON wallets USING gin(data) WHERE deleted_at IS NULL;
...
CREATE TRIGGER wallets_timestamp_trigger
