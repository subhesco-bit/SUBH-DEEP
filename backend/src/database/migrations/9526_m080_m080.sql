-- Folded from backend/src/modules/M080/model.sql (AUDIT_DB.md Finding 11/13 remediation)
-- ﻿-- Water Analytics Schema (M080) / -- Advanced water data analytics, dashboards, and predictive insights
...
CREATE INDEX IF NOT EXISTS idx_water_analytics_location ON water_analytics(location_id);

CREATE INDEX IF NOT EXISTS idx_water_analytics_period ON water_analytics(period_from, period_to);

CREATE INDEX IF NOT EXISTS idx_water_dashboards_user ON water_dashboards(user_id);

CREATE INDEX IF NOT EXISTS idx_water_predictions_location ON water_predictions(location_id);

CREATE INDEX IF NOT EXISTS idx_water_anomaly_alerts_location ON water_anomaly_alerts(location_id);

CREATE INDEX IF NOT EXISTS idx_water_anomaly_alerts_status ON water_anomaly_alerts(status);
