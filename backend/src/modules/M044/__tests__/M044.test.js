...
describe('M044 Service', () => {
  ...
  describe('CRUD Operations', () => {
    ...
    test('create: Should create new record', async () => {
      const result = await m044Service.create(testData);
    ...
    test('getById: Should retrieve created record', async () => {
      const result = await m044Service.getById(createdId);
      expect(result.id).toBe(createdId);
      expect(result.user_id).toBe(testData.user_id);
    });

    test('update: Should update record', async () => {
      const updated = await m044Service.update(createdId, { status: 'inactive' });
      expect(updated.status).toBe('inactive');
    });

    test('delete: Should soft-delete record', async () => {
      await m044Service.delete(createdId);
      await expect(m044Service.getById(createdId)).rejects.toThrow();
    });
  });

  describe('Pagination & Filtering', () => {
    test('getAll: Should return paginated results', async () => {
      const result = await m044Service.getAll({ page: 1, limit: 10 });
    ...
    test('getAll: Should filter by status', async () => {
      const result = await m044Service.getAll({ status: 'active' });
      expect(result.data).toBeInstanceOf(Array);
    });

    test('getAll: Should filter by user_id', async () => {
      const result = await m044Service.getAll({ user_id: 'test-user-123' });
      expect(result.data).toBeInstanceOf(Array);
    });
  });
  ...
  describe('Search', () => {
    test('search: Should find records by query', async () => {
      const result = await m044Service.search('test');
  ...
  describe('Error Handling', () => {
    test('Should throw error for invalid ID', async () => {
      await expect(m044Service.getById('invalid')).rejects.toThrow();
    });

    test('Should throw error for missing user_id', async () => {
      await expect(m044Service.create({})).rejects.toThrow();
    });
  });
});