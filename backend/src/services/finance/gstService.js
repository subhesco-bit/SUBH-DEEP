...
const enterpriseAccounting = require('./enterpriseAccountingService');
...
const GST_RETURN_TRANSITIONS = Object.freeze({
  pending: new Set(['prepared', 'cancelled']), prepared: new Set(['filed', 'cancelled']),
  filed: new Set(['accepted', 'rejected']), rejected: new Set(['prepared']),
  accepted: new Set(), cancelled: new Set(),
});
const GST_PAYMENT_TRANSITIONS = Object.freeze({
  pending: new Set(['processing', 'failed', 'cancelled']), processing: new Set(['completed', 'failed']),
  failed: new Set(['pending', 'cancelled']), completed: new Set(), cancelled: new Set(),
});
const STATE_CODES = new Set([...Array.from({ length: 38 }, (_, i) => String(i + 1).padStart(2, '0')), '97']);
const STATE_ALIASES = new Map(Object.entries({
  JAMMUANDKASHMIR: '01', HIMACHALPRADESH: '02', PUNJAB: '03', CHANDIGARH: '04', UTTARAKHAND: '05',
  HARYANA: '06', DELHI: '07', RAJASTHAN: '08', UTTARPRADESH: '09', BIHAR: '10', SIKKIM: '11',
  ARUNACHALPRADESH: '12', NAGALAND: '13', MANIPUR: '14', MIZORAM: '15', TRIPURA: '16',
  MEGHALAYA: '17', ASSAM: '18', WESTBENGAL: '19', JHARKHAND: '20', ODISHA: '21', CHHATTISGARH: '22',
  MADHYAPRADESH: '23', GUJARAT: '24', DAMANANDDIU: '25', DADRAANDNAGARHAVELI: '26', MAHARASHTRA: '27',
  ANDHRAPRADESH: '37', KARNATAKA: '29', GOA: '30', LAKSHADWEEP: '31', KERALA: '32', TAMILNADU: '33',
  PUDUCHERRY: '34', ANDAMANANDNICOBARISLANDS: '35', TELANGANA: '36', LADAKH: '38', OTHERTERRITORY: '97',
  AS: '18', WB: '19', MH: '27', KA: '29', KL: '32', TN: '33', TS: '36', AP: '37', DL: '07', UP: '09',
}));

function assertMoney(value, fieldName, { allowZero = true } = {}) {
  const number = Number(value);
  if (!Number.isFinite(number) || number < 0 || (!allowZero && number === 0)) {
    const error = new Error(`${fieldName} must be ${allowZero ? 'a non-negative' : 'a positive'} number`);
...
const normalizeState = (value) => {
  ...
  if (STATE_CODES.has(normalized)) return normalized;
  return STATE_ALIASES.get(normalized.replace(/[^A-Z]/g, '')) || normalized;
};
function gstinCheckCharacter(value) {
  const alphabet = '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ';
  let factor = 2; let sum = 0;
...
class GSTService {
  ...
  getGSTRate(productCategory) {
    ...
    return gstRates[productCategory] ?? 18; // Preserve legitimate NIL rates.
  }
  ...
  calculateTaxComponents({ taxableValue, rate, supplierState, placeOfSupply, exempt = false, reverseCharge = false }) {
    const value = assertMoney(taxableValue, 'taxableValue');
    const applicableRate = exempt ? 0 : assertMoney(rate, 'rate');
    if (applicableRate > 100) { const e = new Error('rate must not exceed 100'); e.statusCode = 400; throw e; }
    const supplier = normalizeState(supplierState); const destination = normalizeState(placeOfSupply);
    if (!supplier || !destination) { const e = new Error('supplierState and placeOfSupply are required'); e.statusCode = 400; throw e; }
    const totalTax = r2(value * applicableRate / 100); const intraState = supplier === destination;
    const cgst = intraState ? r2(totalTax / 2) : 0;
    const sgst = intraState ? r2(totalTax - cgst) : 0;
    return { taxableValue: value, rate: applicableRate, supplyType: intraState ? 'intra_state' : 'inter_state',
      cgst, sgst, igst: intraState ? 0 : totalTax, totalTax, total: r2(value + totalTax),
      exempt: Boolean(exempt), reverseCharge: Boolean(reverseCharge),
      taxLiabilityParty: reverseCharge ? 'recipient' : 'supplier' };
  }

  async resolveEffectiveRate({ hsnCode, productCategory, supplyDate = new Date().toISOString().slice(0, 10) }) {
    if (!/^\d{2,8}$/.test(String(hsnCode || ''))) { const e = new Error('hsnCode must contain 2 to 8 digits'); e.statusCode = 400; throw e; }
    const { rows } = await this.pool.query(
      `SELECT hsn_code, product_category, gst_rate, effective_date, expiry_date, description FROM gst_rates
       WHERE hsn_code=$1 AND is_active=TRUE AND effective_date <= $2::date
       AND (expiry_date IS NULL OR expiry_date >= $2::date) AND ($3::text IS NULL OR product_category=$3)
       ORDER BY effective_date DESC, id DESC LIMIT 1`, [String(hsnCode), supplyDate, productCategory || null]);
    if (!rows[0]) { const e = new Error(`No effective GST rate found for HSN ${hsnCode} on ${supplyDate}`); e.statusCode = 422; throw e; }
    return { ...rows[0], gst_rate: Number(rows[0].gst_rate), source: 'gst_rates_effective_date' };
  }

  async createReturn(input, actorId) {
    const gstin = String(input.taxpayerGstNumber || '').trim().toUpperCase();
    if (!this.validateGSTNumber(gstin)) { const e = new Error('A valid taxpayer GSTIN is required'); e.statusCode = 400; throw e; }
    if (!/^(0[1-9]|1[0-2])\d{4}$/.test(String(input.returnPeriod || ''))) { const e = new Error('returnPeriod must be MMYYYY'); e.statusCode = 400; throw e; }
    if (!['GSTR-1', 'GSTR-3B'].includes(input.returnType)) { const e = new Error('returnType must be GSTR-1 or GSTR-3B'); e.statusCode = 400; throw e; }
    const { rows } = await this.pool.query(
      `INSERT INTO gst_returns (return_period,return_type,taxpayer_gst_number,taxpayer_name,taxpayer_state,due_date,total_turnover,total_tax_liability,return_status,filed_by)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,'pending',$9) RETURNING *`,
      [input.returnPeriod, input.returnType, gstin, input.taxpayerName || null, input.taxpayerState || null,
        input.dueDate, assertMoney(input.totalTurnover, 'totalTurnover'), assertMoney(input.totalTaxLiability, 'totalTaxLiability'), actorId || null]);
    return rows[0];
  }

  async transitionReturn(returnId, nextStatus, details = {}, actorId) {
    return withTransaction(async (client) => {
      const found = await client.query('SELECT * FROM gst_returns WHERE id=$1 FOR UPDATE', [returnId]); const current = found.rows[0];
      if (!current) { const e = new Error('GST return not found'); e.statusCode = 404; throw e; }
      if (!GST_RETURN_TRANSITIONS[current.return_status]?.has(nextStatus)) { const e = new Error(`Invalid GST return transition: ${current.return_status} -> ${nextStatus}`); e.statusCode = 409; throw e; }
      if (nextStatus === 'filed' && !details.acknowledgmentNumber) { const e = new Error('acknowledgmentNumber is required when filing'); e.statusCode = 400; throw e; }
      const { rows } = await client.query(
        `UPDATE gst_returns SET return_status=$1,acknowledgment_number=COALESCE($2,acknowledgment_number),filed_by=COALESCE($3,filed_by),
         filing_date=CASE WHEN $1='filed' THEN CURRENT_DATE ELSE filing_date END,updated_at=CURRENT_TIMESTAMP WHERE id=$4 RETURNING *`,
        [nextStatus, details.acknowledgmentNumber || null, actorId || details.filedBy || null, returnId]); return rows[0];
    }, { name: 'gstService.transitionReturn' });
  }

  async createPayment(input, actorId) {
    if (!['CGST', 'SGST', 'IGST', 'CESS', 'INTEREST', 'PENALTY'].includes(input.taxType)) { const e = new Error('Unsupported taxType'); e.statusCode = 400; throw e; }
    const { rows } = await this.pool.query(
      `INSERT INTO gst_payments (return_id,payment_type,tax_type,amount,payment_date,payment_method,transaction_id,payment_status,created_by)
       VALUES ($1,$2,$3,$4,$5,$6,$7,'pending',$8) RETURNING *`,
      [input.returnId || null, input.paymentType, input.taxType, assertMoney(input.amount, 'amount', { allowZero: false }),
        input.paymentDate || null, input.paymentMethod || null, input.transactionId || null, actorId || null]); return rows[0];
  }

  async transitionPayment(paymentId, nextStatus, details = {}) {
    return withTransaction(async (client) => {
      const found = await client.query('SELECT * FROM gst_payments WHERE id=$1 FOR UPDATE', [paymentId]); const current = found.rows[0];
      if (!current) { const e = new Error('GST payment not found'); e.statusCode = 404; throw e; }
      if (!GST_PAYMENT_TRANSITIONS[current.payment_status]?.has(nextStatus)) { const e = new Error(`Invalid GST payment transition: ${current.payment_status} -> ${nextStatus}`); e.statusCode = 409; throw e; }
      if (nextStatus === 'completed' && !details.challanNumber) { const e = new Error('challanNumber is required for a completed payment'); e.statusCode = 400; throw e; }
      const { rows } = await client.query(
        `UPDATE gst_payments SET payment_status=$1,challan_number=COALESCE($2,challan_number),bank_name=COALESCE($3,bank_name),
         branch_name=COALESCE($4,branch_name),updated_at=CURRENT_TIMESTAMP WHERE id=$5 RETURNING *`,
        [nextStatus, details.challanNumber || null, details.bankName || null, details.branchName || null, paymentId]); return rows[0];
    }, { name: 'gstService.transitionPayment' });
  }
   ...
   */
  async postGSTInvoiceToLedger(client, { companyId, invoiceId, invoiceNumber, invoiceDate, actorId,
    ...
    const journal = await enterpriseAccounting.createAndPostJournalWithClient(client, {
      companyId,
      entryNumber,
      entryDate: invoiceDate,
      journalType: 'sales',
      description: `Output GST on GST invoice ${invoiceNumber}`,
      referenceType: 'gst_invoice',
      referenceId: String(invoiceId),
      idempotencyKey: `gst-invoice:${invoiceId}`,
      actorId,
      lines: [
        { accountId: arAccountId, debit: totalGST, credit: 0, description: `GST receivable component - invoice ${invoiceNumber}` },
        ...creditLines.map((line) => ({ accountId: line.accountId, debit: 0, credit: line.amount,
          description: `Output ${line.label} payable - invoice ${invoiceNumber}` })),
      ],
    });

    return { posted: true, journalEntryId: journal.id, entryNumber, totalGST, replayed: Boolean(journal.replayed) };
  }
  ...
  validateGSTNumber(gstNumber) {
    ...
    return gstRegex.test(normalized) && STATE_CODES.has(normalized.slice(0, 2)) &&
      gstinCheckCharacter(normalized.slice(0, 14)) === normalized[14];
  }
