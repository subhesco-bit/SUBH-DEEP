...
async function generateOfflinePaymentQR(merchantId, amount, reference, expiryHours = 24) {
  try {
    ...
    const signature = crypto
      .createHmac('sha256', process.env.OFFLINE_PAYMENT_SECRET || 'default-secret')
...
async function processOfflinePayment(paymentCode, payerId, pin, biometricData = null) {
  try {
    ...
    // BR-08 (ported from origin/claude/todo-list-completion-uosl90, commit
    // 086f494c): recording the transaction, closing out the payment request,
    // and queuing it for sync must all land together — if the status update
    // was lost after the transaction row committed, the same payment_code
    // could be replayed (it is still 'pending'), double-spending the payer.
    // No external call is involved (PIN check and daily-limit read already
    // happened above), so it is safe to hold in one DB transaction.
...
async function syncPaymentTransaction(transactionId) {
  try {
    ...
    // BR-08 (ported from origin/claude/todo-list-completion-uosl90, commit
    // a5cbe65f): creating the online ledger transaction, marking the offline
    // transaction synced, and moving both wallet balances are one economic
    // event (money leaving the payer's wallet and landing in the
    // merchant's). Any partial application here is a real money bug: e.g.
    // debiting the payer without crediting the merchant, or marking the
    // transaction "synced" while the wallets never moved so it can never be
    // retried. No external call is involved, so it is safe as one boundary.
    await withTransaction(async (client) => {
...
async function processUSSDPayment(ussdCode, userId, pin) {
  try {
    ...
    // BR-08 (ported from origin/claude/todo-list-completion-uosl90): same
    // replay/double-spend risk as processOfflinePayment above — the
    // transaction insert, request status update, and sync-queue entry must
    // commit together.
    await withTransaction(async (client) => {
      await client.query(
        `INSERT INTO offline_transactions
