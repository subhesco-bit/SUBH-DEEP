...
export default function M092Page() {
  ...
  const fetchData = useCallback(async (page = 1) => {
    ...
    try {
      ...
      const response = await fetch(`/api/m092?${params}`);
  ...
  const handleSearch = useCallback(async (e) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`/api/m092/search?q=${encodeURIComponent(searchQuery)}`);
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    try {
      ...
      const url = editingId ? `/api/m092/${editingId}` : `/api/m092`;
  ...
  const handleDelete = useCallback(async (id) => {
    ...
    try {
      const response = await fetch(`/api/m092/${id}`, { method: 'DELETE' });
