...
export default function M151Page() {
  ...
  const fetchData = useCallback(async (page = 1) => {
    ...
    try {
      ...
      const response = await fetch(`/api/m151?${params}`);
  ...
  const handleSearch = useCallback(async (e) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`/api/m151/search?q=${encodeURIComponent(searchQuery)}`);
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    try {
      ...
      const url = editingId ? `/api/m151/${editingId}` : `/api/m151`;
  ...
  const handleDelete = useCallback(async (id) => {
    ...
    try {
      const response = await fetch(`/api/m151/${id}`, { method: 'DELETE' });
