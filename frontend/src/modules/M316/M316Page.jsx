...
export default function M316Page() {
  ...
  const fetchData = useCallback(async (page = 1) => {
    ...
    try {
      ...
      const response = await fetch(`/api/m316?${params}`);
  ...
  const handleSearch = useCallback(async (e) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`/api/m316/search?q=${encodeURIComponent(searchQuery)}`);
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    try {
      ...
      const url = editingId ? `/api/m316/${editingId}` : `/api/m316`;
  ...
  const handleDelete = useCallback(async (id) => {
    ...
    try {
      const response = await fetch(`/api/m316/${id}`, { method: 'DELETE' });
