...
test("the auth schema ships outside the globbed directory", () => {
  ...
  assert.ok(pending.length >= 5, `app schema should be pending on a fresh db, got ${pending.length}`);
  assert.ok(pending.every((p) => p.name.endsWith(".sql") && !String(p.path).includes("auth/")));
  assert.ok(pending.some((p) => p.name === "0002_lattice_organism.sql"));
  assert.ok(pending.some((p) => p.name === "0006_schema_integrity.sql"));
